...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/resources/css/board/board_list.css">
<script
...
<body>
	...
	<main>
		<form class="search_form">
			...
			</select> <input class="keyword" name="keyword" id="keyword" value="">
			<input type="submit" class="search_button" value="&#xf002" onclick="search(this.form)">
		</form>
		<div class="board_buttons">
			<div class="radios">
				<label class="orderby"><input type="radio" name="orderby"
					value="B_IDX" onchange="selChange()"
					<c:if test="${paging.orderby eq 'B_IDX'}">checked</c:if>>
					최신순</label> <label class="orderby"><input type="radio"
					name="orderby" value="B_THUMBUP" onchange="selChange()"
					<c:if test="${paging.orderby eq 'B_THUMBUP'}">checked</c:if>>
					좋아요순</label> <label class="orderby"><input type="radio"
					name="orderby" value="B_READHIT" onchange="selChange()"
					<c:if test="${paging.orderby eq 'B_READHIT'}">checked</c:if>>
					조회순</label> <label class="orderby"><input type="radio"
					name="orderby" value="B_REPLY_COUNT" onchange="selChange()"
					<c:if test="${paging.orderby eq 'B_REPLY_COUNT'}">checked</c:if>>
					댓글순</label>
			</div>
			<div>
				<!-- 옵션선택 -->
				<select class="cntpage" id="cntPerPage" name="sel"
					onchange="selChange()">
					<option value="5"
		...
		<c:choose>
			<c:when test="${not empty b_list}">
				<c:forEach var="vo" items="${b_list}">
					<div class="b_container">
						<div class="b_wrapper" onclick="location.href='board_view.do?b_idx=${vo.b_idx}'">
							<div class="b_title">${vo.b_title}
								<label class="label"> <c:if
										test="${vo.b_category eq '강아지'}">
										<i class="fa-solid fa-dog"></i>
									</c:if> <c:if test="${vo.b_category eq '고양이'}">
										<i class="fa-solid fa-cat"></i>
									</c:if> <c:if test="${vo.b_category eq '기타'}">
										<i class="fa-solid fa-guitar"></i>
									</c:if> ${vo.b_category}
								</label>
							</div>
							<div class="b_content"
								onclick="location.href='board_view.do?b_idx=${vo.b_idx}'">${vo.b_content}</div>

						</div>
						<div class="img_wrapper">
							<c:if test="${not empty vo.b_img}">
								<img class="b_img"
									src="${pageContext.request.contextPath}/resources/upload/${vo.b_img}">
							</c:if>
						</div>
					</div>
					<div class="b_info">
			...
			<c:when test="${empty b_list}">
				<div
					style="text-align: center; color: gray; margin: 50px auto 70px;">작성된
					게시글이 없습니다.</div>
				<hr class="first_hr">
			</c:when>
		</c:choose>
