...
<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/cart/payment.css">
...
<script>
	...
	function kakaopay() {
		...
		IMP.request_pay({
			...
			name : '결제테스트',
			amount : '${total_price}', //실제로는 결제 건마다 금액 달라야함★★★
		...
		//buyer_postcode: '123-456',
			...
			if (rsp.success) {
				...
				location.href = "purchase_complete.do?o_addr=" + o_addr; //구매내역 창으로 감 
...
<body>
	...
	<main>
		<form class="payment_form">
			...
			<div class="payment_info">상품명 : ${list[0].p_name} 외 ${fn:length(list) - 1}건</div>
			<div class="payment_info">총 금액 : <fmt:formatNumber value='${total_price}' pattern="#,##0" />원</div>
			<hr class="hr">

			<div class = "item_btn">
				<div>
					<button class = "address_modify_btn" type="button" onclick="location.href='member_info_modify_form.do'">배송지수정</button>
				</div>
			</div>
			<div class = "item_btn">
				<div class = "payment">
					<img src="resources/images/payment_icon2.png" onclick="kakaopay()" style="cursor: pointer;">
				</div> 				
				<div>
					<button class = "continue" type="button" onclick="location.href='product_list.do'">계속 쇼핑하기</button>
				</div>
			</div>
		</form>
	</main>
